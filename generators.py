...
def single_well_generator(total_time,time_step, x0_mean, zeta, k, x_mu,T,
    ...
    diffusion = torch.sqrt(torch.tensor(2*boltzmann*T/zeta))
    dt_tensor = torch.tensor(time_step, device=device)
    positions = torch.zeros((num_of_simulations,num_time_steps))  # use default float32
    positions[:, 0] = x0
    times = torch.arange(0, total_time, time_step, device=device)

    dw = torch.normal(mean=0, std=np.sqrt(time_step), size=(num_of_simulations, num_time_steps-1))  # sample brownian motion increments
